...
fun PalFinderApp(current_loc: Location?, modifier: Modifier = Modifier) {
    ...
    // User location, default location set to a location in singapore for testing
    ...
    val customVibrationLong =
        remember { VibrationEffect.createOneShot(100, VibrationEffect.DEFAULT_AMPLITUDE) }


    // The surface on which all components are drawn
    Surface(modifier) {
        // If the current location is not (yet) available, initialize map to a default location (singapore)
        if (current_loc == null) {
            MapsComposable(
                user_loc,
                destination,
            ...
            // initialize the map and pan to current location
            ...
            MapsComposable(user_loc, destination, waypoints, navigationRunning,
                // If user selects a point of interest on the map, navigate to there
                onDestinationSelected = {
                    destination = it
        ...
        Text(
            text = "navigate",
            modifier.absolutePadding(left = 180.dp, right = 5.dp, top = 0.dp, bottom = 0.dp)
        )
        Switch(
            ...
            run {
                ...
                destination_selected.latLng?.let { selectedLocation ->
                    ...
                    if (current_loc != null) {
                        ...
                        // reset next waypoint to the first waypoint
                        nextWaypointIndex = 0
                    }
                }
            }
        ...
        if (!waypoints.isEmpty()) {
            // If the distance to a waypoint is smaller than 8meters ->
            ...
            if (distanceLatLng(
                ...
                // If last waypoint -> destination is reached, end navigation
                if (nextWaypointIndex == waypoints.lastIndex) {
                    nextWaypointIndex = 0
                    waypoints = emptyList()

                // Otherwise navigate to next waypoint
                } else {
                    nextWaypointIndex += 1
                }
            }
        ...
        if (navigationRunning) {
            // Tactile navigation
            JoystickController { x: Float, y: Float, azimuth ->
                if (!waypoints.isEmpty()) {
